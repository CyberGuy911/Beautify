...
<task type="auto">
  ...
  <action>
    ...
    **Authentication fallback:** If `vercel` CLI commands fail with authentication errors
    (e.g., "Error: Not authenticated" or login prompts), create a checkpoint:human-action
    task asking the user to:
    1. Run `vercel login` and complete browser authentication
    2. Or deploy via Vercel Dashboard (import Git repo at vercel.com/new)
    Then retry CLI commands or proceed with manual deployment confirmation.
  </action>
  <verify>
