...
<task type="auto">
  <name>Task 2: Add transition animations, update download branding, and integrate BeforeAfterSlider</name>
  ...
  <action>
    ...
    2. Update `components/upload-zone.tsx`:
       ...
       a. Add import at top of file with other imports:
          ```typescript
          import { BeforeAfterSlider } from '@/components/before-after-slider'
          ```

       b. Change download filename from `transformed-${fileName}` to `MsFrozen-${baseName}.jpg`
       ...
       c. Replace the img element at lines 238-242 with conditional rendering:
          CURRENT CODE (around lines 238-242):
          ```typescript
          <img
       ...
       d. Apply animate-fade-in to key elements (preview container, buttons)
  ...
  <verify>
    - `grep "import.*BeforeAfterSlider" components/upload-zone.tsx` confirms import added
    - `grep "BeforeAfterSlider beforeSrc" components/upload-zone.tsx` confirms component usage with props
    - `grep "MsFrozen-" components/upload-zone.tsx` confirms download branding
  ...
  <done>Download uses MsFrozen branding, BeforeAfterSlider imported and integrated to show after transformation, smooth animations applied, reset clears all state including slider</done>
</task>
